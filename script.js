    ...
    contactForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
          ...
          body: JSON.stringify({
            websiteId: '5a7802b2-6033-4521-9a75-9d8a5620b902',
            name: data.name || '',
            email: data.email || '',
        ...
        if (response.ok) {
          ...
          console.log('⚠️ Zappy: Server returned non-OK status', response.status);
        }
      } catch (error) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "חפר סטאפס משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
